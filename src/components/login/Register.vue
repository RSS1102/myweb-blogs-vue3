      ...
      <div class="Form_class">
        ...
        >
          ...
          <el-form-item label="验证码" class="my-item" prop="code">
            ...
            <span
              class="login-code"
              style="position: absolute; right: 0; top: 4px; left: 155px"
              @click="btn_code"
            >
          ...
          <el-form-item>
            <el-button type="primary" @click="submitForm('reAccount')"
              >注册</el-button
            >
            <el-button @click="submitFormCancel" style="margin-left: 66px"
              >取消</el-button
            >
          </el-form-item>
        </el-form>
...
<script>
import Sidentify from "../repeat/BaseSidentify";
...
export default {
  ...
  data() {
    ...
    const reEmail = (rule, value, callback) => {
      if (
        /^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{3,6})$/.test(value) ||
        !value
      ) {
        callback();
      } else {
    ...
    return {
      ...
      rules: {
        ...
        userPwds: [{ validator: repassword, trigger: "blur" }],
  ...
  methods: {
    ...
    submit: async function () {
      ...
      //toLowerCase转换小写
      if (
        this.reAccount.code.toLowerCase() !== this.identifyCode.toLowerCase()
      ) {
        console.log("验证码错误");
        //重新刷新验证码
        this.refreshCode();
